...
<div class="container-fluid text-center" id="main-div">    
  <div class="row content">
    <div class="col-md-12 text-left"> 
	  ...
	  <a href="" class="btn btn-primary btn-lg">Add New Car</a>
	  <br><br><br>
		...
		if(typeof(EventSource)!==undefined)
		{
			...
			source.onmessage = function(event)
			{
				...
				for(var i=0; i<JSONObject.length; i++)
				{
					...
					html +='<td>'+JSONObject[i].car_image+'</td>';
					html +='<td>'+JSONObject[i].mfg_date+'</td>';
					html +='<td>'+JSONObject[i].lat+'</td>';					
					html +='<td>'+JSONObject[i].lng+'</td>';
					html +='<td><a href="action.php?act=edit&id='+JSONObject[i].id+'" class="btn btn-primary">Edit</a> <a href=href="action.php?act=delete&id='+JSONObject[i].id+' class="btn btn-danger">Delete</a></td></tr>';
